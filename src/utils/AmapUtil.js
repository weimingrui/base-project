...
AMapUtil.prototype = {
    ...
    drawpanelLine(item, el) {
        ...
        if (cleft > 0) {
            ...
            that.find(".u-panelineb").attr("style", "width:" + Math.abs(cleft) + "px;left:0");
        }
        if (ctop > 0) {
            that.find(".u-panelinea").attr("style", "height:" + ctop + "px;bottom:50%");
            that.find(".u-panelineb").css("bottom", "initial");
        } else {
            that.find(".u-panelinea").attr("style", "height:" + Math.abs(ctop) + "px;top:50%");
